...
public class HomeController {
    ...
    private final LikesRepository likesRepository; // LikeRepository 대신 LikesRepository 하나만 사용하세요
    ...
    public String writeForm(HttpSession session, Model model) {
        ...
        model.addAttribute("loginId", loginId);
        List<String> categoryList = Arrays.asList("스터디", "운동", "친목", "취업", "기타");
        model.addAttribute("categoryList", categoryList);
    ...
    public String savePost(@RequestParam("title") String title,
        ...
        if (imageFile != null && !imageFile.isEmpty()) {
            try {
                post.setImagePath(s3Service.uploadFile(imageFile));
            } catch (IOException e) { 
    ...
    // ... 나머지 PostMapping 메서드들은 동일하게 유지하세요 ...
